...
const SizeL = (props) => {
  ...
  useEffect(() => {
    props.OnLHandler(Data);
  }, [Data]);

  useEffect(() => {
    sizeDetail.map((e) => {
      document.getElementById(`${e}L`).value = "";
      props.OnLHandler([]);
    });
  }, [sizeDetail]);
  useEffect(() => {
    sizeDetail.map((e) => {
      if (!props.IsLactive) document.getElementById(`${e}L`).value = "";
    });
  }, [props.IsLactive]);
      ...
      <div className="form-inline flex-col xl:flex-row mt-2 pt-2 first:mt-0 first:pt-0">
        <div className="form-label xl:w-32 xl:!mr-10">
          <div className="flex text-center">
            <span className="form-control shadow-none">L</span>
          </div>
        </div>
                ...
                : props.SizeDeCount.map((i) => {
                    return (
                      <div key={i} className="sm:flex  items-center mt-5  first:mt-0">
                        ...
                        />
                        <span id={`${sizeDetail[i - 1]}Ldanger`} className={`text-danger ml-3 hidden`}>
                          โปรดใส่แค่ตัวเลข
                        </span>
                      </div>
                    );
