...
const SizeXXXL = (props) => {
  ...
  useEffect(() => {
    props.OnXXXLHandler(Data);
  }, [Data]);

  useEffect(() => {
    sizeDetail.map((e) => {
      document.getElementById(`${e}XXXL`).value = "";
      props.OnXXXLHandler([]);
    });
  }, [sizeDetail]);
  ...
  useEffect(() => {
    sizeDetail.map((e) => {
      if (!props.IsXXXLactive) document.getElementById(`${e}XXXL`).value = "";
    });
  }, [props.IsXXXLactive]);
      ...
      <div className="form-inline flex-col xl:flex-row mt-2 pt-2 first:mt-0 first:pt-0">
        <div className="form-label xl:w-32 xl:!mr-10">
          <div className="flex text-center">
            <span className="form-control shadow-none">XXXL</span>
          </div>
        </div>
                ...
                : props.SizeDeCount.map((i) => {
                    return (
                      <div key={i} className="sm:flex  items-center mt-5  first:mt-0">
                        ...
                        <input
                          id={`${sizeDetail[i - 1]}XXXL`}
                          type="text"
                          className="form-control "
                          placeholder="36.00"
                          onChange={DataHandler}
                        />
                        <span id={`${sizeDetail[i - 1]}XXXLdanger`} className={`text-danger ml-3 hidden`}>
                          โปรดใส่แค่ตัวเลข
                        </span>
                      </div>
                    );
