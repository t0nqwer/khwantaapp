...
import logoUrl from "@/assets/images/logopng-white.png";
import { faker as $f } from "@/utils";
import * as $_ from "lodash";
import classnames from "classnames";
import PropTypes from "prop-types";
import DarkModeSwitcher from "@/components/dark-mode-switcher-new/Main";
      ...
      >
        <div className="h-full flex items-center">
          ...
          <Link to="/" className="logo -intro-x hidden md:flex xl:w-[180px] block">
            <img alt="Enigma Tailwind HTML Admin Template" className="logo__image xl:w-full p-4 md:w-32" src={logoUrl} />
          </Link>
          {/* END: Logo */}
          {/* BEGIN: Breadcrumb */}
          <nav aria-label="breadcrumb" className="-intro-x h-[45px] mr-auto"></nav>
          {/* END: Breadcrumb */}
          <DarkModeSwitcher />
          {/* BEGIN: Search */}
              ...
              <div className="search-result__content">
                ...
                <div className="mb-5">
                  {$_.take($f(), 4).map((faker, fakerKey) => (
                    <a key={fakerKey} href="" className="flex items-center mt-2">
                      ...
                      <div className="ml-auto w-48 truncate text-slate-500 text-xs text-right">{faker.users[0].email}</div>
                    </a>
                  ))}
                </div>
                ...
                {$_.take($f(), 4).map((faker, fakerKey) => (
                  <a key={fakerKey} href="" className="flex items-center mt-2">
                    ...
                    <div className="ml-auto w-48 truncate text-slate-500 text-xs text-right">{faker.products[0].category}</div>
                  </a>
                ))}
              </div>
            </div>
          ...
          <Dropdown className="intro-x mr-4 sm:mr-6">
            <DropdownToggle tag="div" role="button" className="notification notification--bullet cursor-pointer">
                  ...
                  >
                    ...
                    <div className="ml-2 overflow-hidden">
                      <div className="flex items-center">
                        ...
                        <div className="text-xs text-slate-400 ml-auto whitespace-nowrap">{faker.times[0]}</div>
                      </div>
          ...
          <Dropdown className="intro-x w-8 h-8">
            <DropdownToggle tag="div" role="button" className="w-8 h-8 rounded-full overflow-hidden shadow-lg image-fit zoom-in">
            ...
            <DropdownMenu className="w-56">
              <DropdownContent className="bg-primary/80 before:block before:absolute before:bg-black before:inset-0 before:rounded-md before:z-[-1] text-white">
                <DropdownHeader tag="div" className="!font-normal">
                  ...
                  <div className="text-xs text-white/70 mt-0.5 dark:text-slate-500">{$f()[0].jobs[0]}</div>
                </DropdownHeader>
