...
const SizeSS = (props) => {
  ...
  useEffect(() => {
    ...
    props.OnSSHandler(Data);
  }, [Data]);

  useEffect(() => {
    sizeDetail.map((e) => {
      document.getElementById(`${e}SS`).value = "";
      props.OnSSHandler([]);
    });
  }, [sizeDetail]);
  useEffect(() => {
    sizeDetail.map((e) => {
      if (!props.IsSSactive) document.getElementById(`${e}SS`).value = "";
    });
  }, [props.IsSSactive]);
      ...
      <div className="form-inline flex-col xl:flex-row mt-2 pt-2 first:mt-0 first:pt-0">
        <div className="form-label xl:w-32 xl:!mr-10">
          <div className="flex text-center">
            <span className="form-control shadow-none">SS</span>
          </div>
        </div>
              ...
              {sizeDetail.length !== props.SizeDeCount.length
                ...
                return (
                  <div key={i} className="sm:flex  items-center mt-5  first:mt-0">
                    ...
                    <input
                      id={`${sizeDetail[i - 1]}SS`}
                      type="text"
                      className="form-control "
                      placeholder="36.00"
                      onChange={DataHandler}
                    />
                    <span id={`${sizeDetail[i - 1]}SSdanger`} className={`text-danger ml-3 hidden`}>
                      โปรดใส่แค่ตัวเลข
                    </span>
                  </div>
                );
