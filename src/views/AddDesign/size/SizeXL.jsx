...
const SizeXL = (props) => {
  ...
  useEffect(() => {
    props.OnXLHandler(Data);
  }, [Data]);

  useEffect(() => {

    sizeDetail.map((e) => {
      document.getElementById(`${e}XL`).value = "";
      props.OnXLHandler([]);
    });
  }, [sizeDetail]);
  useEffect(() => {
    ...
    sizeDetail.map((e) => {
      if (!props.IsXLactive) document.getElementById(`${e}XL`).value = "";
    });
  }, [props.IsXLactive]);
      ...
      <div className="form-inline flex-col xl:flex-row mt-2 pt-2 first:mt-0 first:pt-0">
        <div className="form-label xl:w-32 xl:!mr-10">
          <div className="flex text-center">
            <span className="form-control shadow-none">XL</span>
          </div>
        </div>
              ...
              {sizeDetail.length !== props.SizeDeCount.length
                ...
                return (
                  <div key={i} className="sm:flex  items-center mt-5  first:mt-0">
                    ...
                    <input
                      id={`${sizeDetail[i - 1]}XL`}
                      type="text"
                      className="form-control "
                      placeholder="36.00"
                      onChange={DataHandler}
                    />
                    <span id={`${sizeDetail[i - 1]}XLdanger`} className={`text-danger ml-3 hidden`}>
                      โปรดใส่แค่ตัวเลข
                    </span>
                  </div>
                );
